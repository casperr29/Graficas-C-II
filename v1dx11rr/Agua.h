...
#pragma region Atributos
	...
	ID3D11ShaderResourceView* colorMapA;
...
#pragma endregion
public:
	Agua(int ancho, int alto, ID3D11Device* D3DDevice, ID3D11DeviceContext* D3DContext)
	{
		...
		CargaParametros(L"TextTerreno\\Agua\\water2.jpg",
			L"TextTerreno\\Agua\\waterNorm.jpg",
			L"TextTerreno\\Agua\\waterRugh2.jpg",
			L"TextTerreno\\terreno.jpg", 100.0f); //Plano con lago
	}
	...
	bool CargaParametros(WCHAR* diffuseTex,WCHAR* NormalTex, WCHAR* SpecTex, WCHAR* heightTex, float tile)
	{
		HRESULT d3dResult;
...
#pragma region Vinculacion de shaders
		...
		bool compileResult = CompileD3DShader(L"Agua1.fx", "VS_Main", "vs_4_0", &vsBuffer);
		...
		compileResult = CompileD3DShader(L"Agua1.fx", "PS_Main", "ps_4_0", &psBuffer);
...
#pragma region Indices y Vertices
...
#pragma region Matriz MVP y Camara
		...
		d3dResult = d3dDevice->CreateBuffer(&constDesc, 0, &wave);
...
#pragma region Transparency
		...
		d3dResult = d3dDevice->CreateBlendState(&transparentDesc, &TransparentBS);
		if (FAILED(d3dResult)) {
			return false;
		}
...
#pragma endregion
	...
	void Draw(D3DXMATRIX vista, D3DXMATRIX proyeccion , D3DXVECTOR3 posCam, FLOAT ondas, float* lightDir, float* lightColor, float posx, float posy, float posz)
	{
		...
		d3dContext->PSSetShaderResources(2, 1, &colorMapS);
		d3dContext->PSSetShaderResources(3, 1, &colorMapA);
		...
		d3dContext->UpdateSubresource(wave, 0, 0, &ondas, sizeof(float), 0);
		d3dContext->UpdateSubresource(lightDirCB, 0, 0, &lightDirF, 0, 0);
		d3dContext->UpdateSubresource(lightColorCB, 0, 0, &lightColorF, 0, 0);
		...
		d3dContext->VSSetConstantBuffers(4, 1, &wave);
		d3dContext->VSSetConstantBuffers(5, 1, &lightDirCB);
		d3dContext->VSSetConstantBuffers(6, 1, &lightColorCB);

		CONST float Trans[4] = { 1.0f,1.0f,1.0f,1.0f };
		d3dContext->OMSetBlendState(TransparentBS, Trans, 1);
