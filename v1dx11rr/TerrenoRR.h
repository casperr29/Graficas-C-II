...
private:
	...
	ID3D11ShaderResourceView* colorMapN2;
	ID3D11ShaderResourceView* colorMapN3;
	ID3D11ShaderResourceView* colorMapS;
	ID3D11ShaderResourceView* colorMapS2;
	ID3D11ShaderResourceView* colorMapS3;
...
public:
	...
	bool CargaParametros(WCHAR* diffuseTex, WCHAR* heightTex, float tile)
	{
		...
		#pragma region Vinculacion de shaders
			...
			bool compileResult = CompileD3DShader( L"Shaders\\Shader.fx", "VS_Main", "vs_4_0", &vsBuffer );
			...
			compileResult = CompileD3DShader( L"Shaders\\Shader.fx", "PS_Main", "ps_4_0", &psBuffer );
		...
		#pragma region Matriz MVP y Camara
			...
			//PosCamera
			d3dResult = d3dDevice->CreateBuffer(&constDesc, 0, &cameraPos);
	...
	void Draw(D3DXMATRIX vista, D3DXMATRIX proyeccion, float* lightDir, float* lightColor, D3DXVECTOR3 posCam)
	{
		...
		d3dContext->PSSetShaderResources( 3, 1, &colorMapN);
		d3dContext->PSSetShaderResources( 4, 1, &colorMapN2);
		d3dContext->PSSetShaderResources( 5, 1, &colorMapN3);
		d3dContext->PSSetShaderResources( 6, 1, &colorMapS);
		d3dContext->PSSetShaderResources( 7, 1, &colorMapS2);
		d3dContext->PSSetShaderResources( 8, 1, &colorMapS3);
		d3dContext->PSSetShaderResources( 9, 1, &blendMap );
		d3dContext->PSSetShaderResources( 10,1, &blendMap2);
		...
		d3dContext->UpdateSubresource(this->lightDirCB, 0, 0, &lightDirF, 0, 0);
		d3dContext->UpdateSubresource(this->lightColorCB, 0, 0, &lightColorF, 0, 0);
		d3dContext->UpdateSubresource(this->cameraPos, 0, 0, &camPos, 0, 0);
		...
		d3dContext->VSSetConstantBuffers(4, 1, &lightColorCB);

		//d3dContext->VSSetConstantBuffers(5, 1, &cameraPos); //Antes el 3
...
private:
	...
	void LoadHeightData(WCHAR* heightTex)
	{
		...
		//obtenemos la cantidad de p�xeles en ancho y profundidad
