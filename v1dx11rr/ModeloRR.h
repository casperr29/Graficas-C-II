...
float* getSphere(float radio)
	...
	bool CargaParametros(char* ModelPath, WCHAR* diffuseTex, WCHAR* specularTex)
	{
		...
		#pragma region Vinculacion de shaders
			...
			bool compileResult = CompileD3DShader(L"Shaders\\Modelo.fx", "VS_Main", "vs_4_0", &vsBuffer);
			...
			compileResult = CompileD3DShader(L"Shaders\\Modelo.fx", "PS_Main", "ps_4_0", &psBuffer);
	...
	void Draw(D3DXMATRIX vista, D3DXMATRIX proyeccion, float ypos, D3DXVECTOR3 posCam, float specForce, float rot, char angle, float scale, float* lightDir, float* lightColor)
	{
		...
		//d3dContext->UpdateSubresource(lightDirCB, 0, 0, lightDir, 0, 0);
