...
public:
	...
	bool CargaParametros(WCHAR* billb, WCHAR* normal, float escala)
	{
		...
		#pragma region Vinculacion de shaders
			...
			bool compileResult = CompileD3DShader(L"Shaders\\Billboard.hlsl", "VS_Main", "vs_4_0", &vsBuffer);
			...
			compileResult = CompileD3DShader(L"Shaders\\Billboard.hlsl", "PS_Main", "ps_4_0", &psBuffer);
		...
		vector2* uv1, vector2* uv2, vector2* uv3, vector2* uv4, int frame, char type, float* lightDir, float* lightColor)
	{
		...
		#pragma region Vertices
			// Se calculan los vertices 'x' y 'z'. 'Y' se saca del mapa de normales
			if (type == 'D') {
				vertices[0].pos.x = 0 * escala * escalax;
				vertices[0].pos.y = 0 * escala * escalay;
				vertices[0].pos.z = -1 * escala;
				vertices[0].UV.x = uv1[frame].u;
				vertices[0].UV.y = uv1[frame].v;

				vertices[1].pos.x = 0 * escala * escalax;
				vertices[1].pos.y = 2 * escala * escalay;
				vertices[1].pos.z = -1 * escala;
				vertices[1].UV.x = uv2[frame].u;
				vertices[1].UV.y = uv2[frame].v;

				vertices[2].pos.x = 0 * escala * escalax;
				vertices[2].pos.y = 2 * escala * escalay;
				vertices[2].pos.z = 1 * escala;
				vertices[2].UV.x = uv3[frame].u;
				vertices[2].UV.y = uv3[frame].v;

				vertices[3].pos.x = 0 * escala * escalax;
				vertices[3].pos.y = 0 * escala * escalay;
				vertices[3].pos.z = 1 * escala;
